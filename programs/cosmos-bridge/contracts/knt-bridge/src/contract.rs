/// Knt Bridge — CosmWasm (Cosmos/IBC) native bridge contract
///
/// Implements cross-chain token bridging between Cosmos ecosystem and Knt.
...
///
/// The MPC path bridges to non-IBC chains (Knt EVM, Solana, Bitcoin, etc.)
...
pub struct Config {
    ...
    pub chain_id: u64, // Cosmos chain identifier in Knt namespace
...
fn build_mint_message(chain_id: u64, nonce: u64, recipient: &str, amount: Uint128) -> Vec<u8> {
    let mut msg = b"KNT_BRIDGE_MINT".to_vec();
...
fn build_release_message(chain_id: u64, nonce: u64, recipient: &str, amount: Uint128) -> Vec<u8> {
    let mut msg = b"KNT_BRIDGE_RELEASE".to_vec();
    msg.extend_from_slice(&chain_id.to_le_bytes());
    msg.extend_from_slice(&nonce.to_le_bytes());
    msg.extend_from_slice(recipient.as_bytes());
    msg.extend_from_slice(&amount.u128().to_le_bytes());
...
mod tests {
    ...
    fn lock_and_bridge_locks_funds_and_increments_nonce() {
        ...
        let info = mock_info(USER, &[Coin::new(10_000u128, "uknt")]);
        let res = execute(
            deps.as_mut(),
            env.clone(),
            info,
    ...
    fn lock_and_bridge_rejects_when_paused() {
        ...
        let info = mock_info(USER, &[Coin::new(1000u128, "uknt")]);
        let err = execute(
    ...
    fn mint_bridged_with_valid_signature() {
        ...
        register_token(&mut deps, "bridged_uknt", 1_000_000);
    ...
    fn burn_bridged_increments_nonce_and_total() {
        ...
        let info = mock_info(USER, &[Coin::new(2_000u128, "bridged_uknt")]);
        let res = execute(
            deps.as_mut(),
            env,
            info,
