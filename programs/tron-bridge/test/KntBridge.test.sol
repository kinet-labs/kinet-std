...
import "../contracts/KntBridge.sol";
...
contract KntBridgeTest is Test {
    KntBridge bridge;
    MockToken token;
    ...
    function setUp() public {
        ...
        bridge = new KntBridge(mpcSigner, 100); // 1% fee
        token = new MockToken();
    ...
    function _signMint(
        ...
        bytes32 digest = keccak256(abi.encodePacked(
            "KNT_BRIDGE_MINT", sourceChainId, nonce, recipient, _token, amount
        ));
    ...
    function test_mint_revertsInvalidSignature() public {
        ...
        bytes32 digest = keccak256(abi.encodePacked(
            "KNT_BRIDGE_MINT", sourceChain, nonce, user, address(token), amount
        ));
        bytes32 ethHash = MessageHashUtils.toEthSignedMessageHash(digest);
