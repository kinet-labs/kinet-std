...
async fn setup() -> (KntBridge<WalletUnlocked>, WalletUnlocked, WalletUnlocked) {
    let num_wallets = 3;
    ...
    let bridge = KntBridge::new(contract_id.clone(), admin_wallet.clone());
...
async fn test_pause_by_non_admin_fails() {
    ...
    let user_bridge = KntBridge::new(bridge.contract_id().clone(), user.clone());
