(* Knt Bridge — Tezos native bridge (CameLIGO)
 *
 ...
 * metals tokens to flow to/from Knt DeFi ecosystem.
 *)
...
let mint_bridged (param : mint_param) (store : storage) : return_type =
  ...
  let () = match Big_map.find_opt nonce_key store.processed_nonces with
    ...
    | _ -> () in
  (* Verify Ed25519 signature *)
  let msg = Bytes.pack ("KNT_BRIDGE_MINT", param.source_chain_id, param.nonce, param.recipient, param.amount) in
