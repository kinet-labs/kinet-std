/// Knt Bridge — Polkadot/Substrate ink! smart contract
///
...
mod knt_bridge {
    ...
    // Polkadot chain ID in Knt namespace
    ...
    mod tests {
        ...
        fn create_bridge() -> KntBridge {
            let accounts = default_accounts();
            set_caller(accounts.alice);
            KntBridge::new(vec![signer_key()], 2, 100) // 1% fee, threshold 2
        }
        ...
        fn initialize_rejects_fee_above_500() {
            let accounts = default_accounts();
            set_caller(accounts.alice);
            KntBridge::new(vec![signer_key()], 2, 501);
        }
