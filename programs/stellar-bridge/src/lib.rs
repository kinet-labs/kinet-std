#![no_std]
// Knt Bridge -- Stellar Soroban native bridge (Rust/WASM)
//
...
impl KntBridge {
    ...
    fn build_mint_message(env: &Env, chain_id: u64, nonce: u64, _recipient: &Address, amount: i128) -> Bytes {
        ...
        msg.extend_from_slice(b"KNT_BRIDGE_MINT");
...
mod tests {
    ...
    fn setup_env() -> (Env, Address, Address) {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register(KntBridge, ());
    ...
    fn init_bridge(env: &Env, contract_id: &Address, admin: &Address, signers: Vec<BytesN<32>>) {
        let client = KntBridgeClient::new(env, contract_id);
    ...
    fn test_mint_bridged_with_valid_signature() {
        ...
        let msg = KntBridge::build_mint_message(&env, source_chain, nonce, &recipient, amount);
