...
/**
 * @title DIDResolver - Universal DID Resolution for Kinet Network
 * @notice Resolves DIDs across multiple registries and methods
 * @dev Supports omnichain identity resolution across Knt, AI, Ethereum, etc.
 *
 * SUPPORTED METHODS:
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │  Method     │ Format                          │ Example                     │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │  knt        │ did:knt:<identifier>            │ did:knt:alice               │
 * │  knt        │ did:knt:mainnet:<address>       │ did:knt:mainnet:0x1234...   │
 * │  knt        │ did:knt:testnet:<address>       │ did:knt:testnet:0x1234...   │
 ...
 * │  web        │ did:web:<domain>                │ did:web:knt.network         │
 * └─────────────────────────────────────────────────────────────────────────────┘
 *
 * RESOLUTION FLOW:
 * ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
...
contract DIDResolver is IDIDResolver, AccessControl {
     ...
     * @param did The DID string (e.g., "did:knt:alice")
     * @return method The DID method (e.g., "knt")
     */
...
contract OmnichainDIDResolver is DIDResolver {
    ...
    function getOmnichainVariants(string calldata did) external view returns (string[] memory variants) {
        ...
        // e.g., did:knt:alice -> [did:ai:alice, did:ethr:0x...]
