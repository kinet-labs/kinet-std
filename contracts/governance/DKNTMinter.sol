...
/**
 * @title DKNTMinter - DAO-Controlled DKNT Emissions
 * @notice Strategic emission controller for DKNT governance token
 * @dev Implements LP-3002 extension for protocol-incentivized DKNT distribution
 *
 * DKNT EMISSION ARCHITECTURE:
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │  DKNT is the actual governance token - minted for strategic activities     │
 * │                                                                             │
 * │  Emission Categories:                                                       │
 * │  ┌───────────────────────────────────────────────────────────────────────┐ │
 * │  │ PROTOCOL EMISSIONS:                                                   │ │
 ...
 * │                                                                             │
 * │  Collateral Backing:                                                        │
 * │  - Users can deposit KNT as collateral to mint DKNT                        │
 ...
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
contract DKNTMinter is AccessControl, Pausable, ReentrancyGuard {
    ...
    struct EmissionConfig {
        uint256 baseAmount; // Base DKNT amount per emission
        uint256 maxAmount; // Max DKNT per single emission
        uint256 cooldown; // Cooldown between emissions for same recipient
        uint256 dailyLimit; // Max DKNT per day for this emission type
        uint256 dailyEmitted; // DKNT emitted today
    ...
    struct CollateralInfo {
        uint256 amount; // Amount of KNT deposited as collateral
        uint256 dkntMinted; // DKNT minted against this collateral
        uint256 depositTime; // When collateral was deposited
    }

    // ============ State ============

    /// @notice The DKNT token
    IDKNT public dknt;

    /// @notice The KNT token (for collateral)
    IERC20 public knt;
    ...
    /// @notice Total DKNT emitted per user (for analytics)
    ...
    event DKNTEmitted(address indexed recipient, bytes32 indexed emissionType, uint256 amount, bytes32 reason);
    ...
    event CollateralDeposited(address indexed user, uint256 kntAmount, uint256 dkntMinted);
    event CollateralWithdrawn(address indexed user, uint256 kntAmount, uint256 dkntBurned);
    ...
    constructor(address _dknt, address _knt, address _treasury, address admin, address dao) {
        if (
            _dknt == address(0) || _knt == address(0) || _treasury == address(0) || admin == address(0)
                || dao == address(0)
        ) {
            revert ZeroAddress();
        }
        ...
        globalDailyLimit = 100_000e18; // 100K DKNT per day global limit
    ...
    /// @notice Deposit KNT as collateral to mint DKNT
    /// @param kntAmount Amount of KNT to deposit
    /// @return dkntMinted Amount of DKNT minted
    function depositCollateral(uint256 kntAmount) external nonReentrant whenNotPaused returns (uint256 dkntMinted) {
        if (kntAmount == 0) revert ZeroAmount();

        // Transfer KNT from user
        knt.safeTransferFrom(msg.sender, address(this), kntAmount);

        // Mint DKNT 1:1
        dkntMinted = kntAmount;

        // Update collateral info
        CollateralInfo storage info = collateral[msg.sender];
        info.amount += kntAmount;
        info.dkntMinted += dkntMinted;
        info.depositTime = block.timestamp;

        totalCollateral += kntAmount;

        // Mint DKNT to user
        dknt.mint(msg.sender, dkntMinted, keccak256("COLLATERAL_DEPOSIT"));

        emit CollateralDeposited(msg.sender, kntAmount, dkntMinted);
    }

    /// @notice Withdraw collateral by burning DKNT
    /// @param dkntAmount Amount of DKNT to burn
    /// @return kntReturned Amount of KNT returned
    function withdrawCollateral(uint256 dkntAmount) external nonReentrant returns (uint256 kntReturned) {
        if (dkntAmount == 0) revert ZeroAmount();

        CollateralInfo storage info = collateral[msg.sender];
        if (dkntAmount > info.dkntMinted) revert InsufficientCollateral();

        // Calculate proportional KNT to return
        kntReturned = (dkntAmount * info.amount) / info.dkntMinted;

        // Update collateral info
        info.amount -= kntReturned;
        info.dkntMinted -= dkntAmount;
        totalCollateral -= kntReturned;

        // User must have DKNT to burn (checked by transfer)
        // Note: In a full implementation, we'd need a burn function on DKNT
        // For now, we transfer to treasury where it gets demurrged
        IERC20(address(dknt)).safeTransferFrom(msg.sender, treasury, dkntAmount);

        // Return KNT
        knt.safeTransfer(msg.sender, kntReturned);

        emit CollateralWithdrawn(msg.sender, kntReturned, dkntAmount);
    }

    // ============ Emission Functions (Called by authorized contracts) ============

    /// @notice Emit DKNT for a strategic activity (called by emitters)
    /// @param recipient Address to receive DKNT
    /// @param emissionType Type of emission
    /// @param amount Amount of DKNT to emit (0 = use baseAmount)
    /// @param reason Additional context (activity hash, tx ID, etc.)
    function emitDKNT(address recipient, bytes32 emissionType, uint256 amount, bytes32 reason)
        external
        ...
        whenNotPaused
    {
        ...
        if (minBackingRatioBps > 0) {
            uint256 newTotalSupply = dknt.totalSupply() + emitAmount;
        ...
        // Emit DKNT
        dknt.mint(recipient, emitAmount, reason);

        emit DKNTEmitted(recipient, emissionType, emitAmount, reason);
    }

    /// @notice Batch emit DKNT to multiple recipients
    /// @param recipients Addresses to receive DKNT
    /// @param emissionType Type of emission
    /// @param amounts Amounts per recipient (0 = use baseAmount)
    /// @param reason Shared reason for batch
    function batchEmitDKNT(
        ...
        if (validCount > 0) {
            ...
            dknt.batchMint(finalRecipients, finalAmounts, reason);
        }
    }
    ...
    /// @notice Emission rate per KNT of fees (1 DKNT per 10 KNT of fees = 10%)
    ...
    function recordFeeEmission(uint8 chainId, uint256 feeAmount) external onlyRole(EMITTER_ROLE) whenNotPaused {
        ...
        uint256 dkntAmount = (feeAmount * feeEmissionRate * multiplier) / (10000 * 10000);

        if (dkntAmount == 0) return;

        // Check global daily limit
        _resetDailyLimits(keccak256("FEE_EMISSION"));
        if (globalDailyEmitted + dkntAmount > globalDailyLimit) {
            dkntAmount = globalDailyLimit - globalDailyEmitted;
        }
        if (dkntAmount == 0) return;

        // Mint to treasury (fees generate DKNT for treasury, not individual users)
        globalDailyEmitted += dkntAmount;
        bytes32 reason = bytes32(uint256(chainId));
        dknt.mint(treasury, dkntAmount, reason);

        emit DKNTEmitted(treasury, keccak256("FEE_EMISSION"), dkntAmount, reason);
    }
    ...
    function backingRatio() external view returns (uint256 ratioBps) {
        uint256 supply = dknt.totalSupply();
    ...
    function getCollateralInfo(address user)
        external
        view
        returns (uint256 kntDeposited, uint256 dkntMinted, uint256 depositTime)
    {
        CollateralInfo memory info = collateral[user];
        return (info.amount, info.dkntMinted, info.depositTime);
    }
    ...
    function _initializeDefaultConfigs() internal {
        ...
        emissionConfigs[EMISSION_VALIDATOR] = EmissionConfig({
            baseAmount: 100e18, // 100 DKNT base
            maxAmount: 1000e18, // Max 1000 DKNT per emission
            cooldown: 1 days, // Daily validator rewards
            dailyLimit: 50_000e18, // 50K DKNT per day
            dailyEmitted: 0,
            lastResetDay: 0,
        ...
        emissionConfigs[EMISSION_BRIDGE] = EmissionConfig({
            baseAmount: 10e18, // 10 DKNT per bridge tx
            maxAmount: 100e18,
        ...
        emissionConfigs[EMISSION_LP] = EmissionConfig({
            baseAmount: 50e18, // 50 DKNT for LP provision
            maxAmount: 500e18,
            cooldown: 1 days,
        ...
        emissionConfigs[EMISSION_STAKING] = EmissionConfig({
            baseAmount: 25e18, // 25 DKNT staking bonus
            maxAmount: 250e18,
        ...
        emissionConfigs[EMISSION_REFERRAL] = EmissionConfig({
            baseAmount: 50e18, // 50 DKNT per referral
            maxAmount: 50e18,
        ...
        emissionConfigs[EMISSION_GRANT] = EmissionConfig({
            baseAmount: 500e18, // 500 DKNT grant base
