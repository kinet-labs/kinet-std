 ...
 * @notice Manages gauge weights based on vKNT voting
 *
 * GAUGE VOTING:
 * ┌─────────────────────────────────────────────────────────────────┐
 * │  vKNT holders vote to allocate weights to gauges               │
 * │                                                                 │
 * │  User voting power = vKNT balance                               │
 ...
 *
 * GAUGES:
 * - BurnGauge: KNT burning (deflationary)
...
contract GaugeController is ReentrancyGuard, Ownable2Step {
    ...
    /// @notice vKNT token for voting power
    IvKNT public immutable vKnt;
    ...
    constructor(address _vKnt) Ownable(msg.sender) {
        vKnt = IvKNT(_vKnt);
    ...
    function vote(uint256 gaugeId, uint256 weight) external nonReentrant {
        ...
        uint256 votingPower = vKnt.balanceOf(msg.sender);
