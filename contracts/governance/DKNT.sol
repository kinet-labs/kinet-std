...
/**
 * @title DKNT - DAO KNT Rebasing Governance Token
 * @notice OHM-style rebasing token with demurrage mechanics
 * @dev Implements LP-3002 Governance Token Stack
 *
 * DKNT TOKENOMICS:
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │  DKNT is a rebasing governance token backed 1:1 by staked KNT              │
 * │                                                                             │
 * │  Properties:                                                                │
 * │  - Backing: 1 KNT = 1 DKNT (redeemable)                                     │
 * │  - Rebase Rate: 0.3-0.5% per epoch (8 hours)                                │
 * │  - Demurrage: 0.1% per day on unstaked DKNT                                 │
 * │  - Transferable: Yes                                                        │
 * │                                                                             │
 * │  Staking Tiers:                                                             │
 ...
 * │  After 365 days: 1000 DKNT → 694.0 DKNT (unstaked)                          │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
contract DKNT is ERC20, ReentrancyGuard, AccessControl {
    ...
    /// @notice Role for emission minting (DKNTMinter)
    ...
    event Unstaked(address indexed user, uint256 amount, uint256 kntReturned);
    ...
    constructor(address _knt, address _treasury, address admin) ERC20("DAO KNT", "DKNT") {
        if (_knt == address(0) || _treasury == address(0) || admin == address(0)) {
            revert ZeroAddress();
        }
    ...
    /// @notice Stake KNT to receive DKNT
    /// @param amount Amount of KNT to stake
    /// @param tier Desired staking tier
    /// @return dkntMinted Amount of DKNT minted
    function stake(uint256 amount, Tier tier) external nonReentrant returns (uint256 dkntMinted) {
    ...
    /// @notice Unstake DKNT to receive KNT
    /// @param amount Amount of DKNT to unstake
    /// @return kntReturned Amount of KNT returned
    function unstake(uint256 amount) external nonReentrant returns (uint256 kntReturned) {
        if (amount == 0) revert ZeroAmount();
        ...
        emit Unstaked(msg.sender, amount, kntReturned);
    }

    /// @notice Redeem DKNT for underlying KNT 1:1 (no lock)
    /// @dev Only works for unstaked DKNT balance
    /// @param amount Amount to redeem
    /// @return kntReturned Amount of KNT returned
    function redeem(uint256 amount) external nonReentrant returns (uint256 kntReturned) {
        if (amount == 0) revert ZeroAmount();
    ...
    /// @return rebased Amount of DKNT rebased
    ...
    /// @notice Mint DKNT for strategic emissions (called by DKNTMinter)
    ...
    /// @notice Batch mint DKNT to multiple recipients
    ...
    function _claimRebase(address account) internal returns (uint256 rebased) {
        ...
        if (rebased > 0) {
            ...
            // Mint rebased DKNT (backed by treasury)
