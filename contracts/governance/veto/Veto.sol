 ...
 * @title Veto
 * @author kinet labs
 * @notice KRC20-based veto voting for parent DAO control
 * @dev Allows token holders to vote to veto a child DAO.
 *
 * Features:
 * - EIP-7201 namespaced storage for upgrade safety
 * - Token-weighted voting via KRC20 (Knt token standard)
...
contract Veto is IVeto, ERC165, Initializable {
     ...
     * @custom:storage-location erc7201:knt.governance.veto
     */
    struct VetoStorage {
        ...
        uint32 vetoPeriod;
        /// @notice KRC20 token used for voting weight
        address votingToken;
     ...
     * @param votingToken_ KRC20 token for voting weight
    ...
    function _getVotingWeight(address voter, uint48 timestamp) internal view virtual returns (uint256) {
        ...
        return IKRC20(_getStorage().votingToken).balanceOf(voter);
    }
