...
/**
 * @title vKNT - Vote-Escrowed KNT
 * @notice Lock KNT to receive voting power for gauge weight allocation
 *
 * VE-TOKENOMICS:
 * ┌─────────────────────────────────────────────────────────────────┐
 * │  Lock KNT for 1 week to 4 years                                 │
 * │  Longer lock = More voting power                                │
 * │                                                                 │
 * │  vKNT = KNT_amount * (lock_time / MAX_LOCK_TIME)                │
 * │                                                                 │
 * │  Example:                                                       │
 * │  - 1000 KNT locked 4 years = 1000 vKNT                          │
 * │  - 1000 KNT locked 1 year  = 250 vKNT                           │
 * │  - 1000 KNT locked 1 week  = ~5 vKNT                            │
 * │                                                                 │
 ...
 */
contract vKNT is ReentrancyGuard {
    ...
    struct LockedBalance {
        uint256 amount; // Locked KNT amount
    ...
    /// @notice Token name
    string public constant name = "Vote-Escrowed KNT";

    /// @notice Token symbol
    string public constant symbol = "vKNT";
    ...
    constructor(address _knt) {
        if (_knt == address(0)) revert ZeroAddress();
        knt = IERC20(_knt);
    ...
    /// @param amount Amount of KNT to lock
    ...
    /// @param amount Additional KNT to lock
    ...
    /// @return Voting power (vKNT balance)
