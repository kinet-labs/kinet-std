...
contract KNTVoting is IVoting, Ownable {
    ...
    function addProposals(string memory newProposal, uint40 startTime, uint40 endTime) public {
        ...
        IERC20(KNT).safeTransferFrom(msg.sender, address(this), (proposalFee * 10 ** uint256(decimals)));
    ...
    function voteProposal(string memory proposal, bool choice) public override {
        ...
        // C-03 fix: Use safeTransferFrom to handle non-standard ERC20 returns
        IERC20(KNT).safeTransferFrom(msg.sender, address(this), (amount * 10 ** uint256(decimals)));
        emit votedProposal(proposal, choice);
    }

    function withdraw() public onlyOwner {
        require(IERC20(KNT).balanceOf(address(this)) > 0, "Balance is 0");
        ...
        IERC20(KNT).safeTransfer(withdrawAddress, IERC20(KNT).balanceOf(address(this)));
        require(os);
    }
}
