 ...
 * BONDING FLOW:
 * =============
 * 1. User deposits whitelisted collateral (KETH, KBTC, CYRUS, etc.)
 ...
 * COLLATERAL TIERS:
 * =================
 * TIER_1: Native ecosystem (KUSD, KETH, KBTC, CYRUS, MIGA, PARS) → 25% discount
 ...
 *
 * NOTE: USD stablecoins NOT RECOMMENDED - use KBTC or KETH instead
 */
...
contract LiquidBond is Ownable, ReentrancyGuard {
    ...
    function _getValueInSats(address token, uint256 amount) internal view returns (uint256) {
        ...
        if (config.priceFeed == address(0)) {
            ...
            // This works for KBTC, for others need proper feed
            return amount;
        }
