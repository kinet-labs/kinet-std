...
/**
 * @title   ConfidentialWKNT.
 * @notice  This contract allows users to wrap/unwrap trustlessly
 *          KNT (or other native tokens) to ConfidentialKRC20 tokens.
 */
abstract contract ConfidentialWKNT is ConfidentialKRC20, IConfidentialKRC20Wrapped, ReentrancyGuard, TFHEApp {
    /// @notice Returned if KNT transfer fails.
    error KNTTransferFail();
    ...
    /**
     * @notice                          Deposit/withdraw KNT (or other native tokens).
    ...
    constructor(uint256 maxDecryptionDelay_)
        ConfidentialKRC20(string(abi.encodePacked("Confidential Wrapped KNT")), string(abi.encodePacked("WKNTc")))
    {
    ...
    /**
     * @notice         Unwrap ConfidentialKRC20 tokens to KNT (or other native tokens).
    ...
    /**
     * @notice Wrap KNT (or other native token) to an encrypted format.
     */
