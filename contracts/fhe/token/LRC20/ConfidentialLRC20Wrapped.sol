...
import { IConfidentialKRC20Wrapped } from "./IConfidentialKRC20Wrapped.sol";
import { ConfidentialKRC20 } from "./ConfidentialKRC20.sol";

/**
 * @title   ConfidentialKRC20Wrapped.
 * @notice  This contract allows users to wrap/unwrap trustlessly KRC20 tokens to ConfidentialKRC20 tokens.
 * @dev     This implementation does not support tokens with rebase functions or tokens with a fee on transfer.
 *          All KRC20 tokens must have decimals superior or equal to 6 decimals.
 */
abstract contract ConfidentialKRC20Wrapped is ConfidentialKRC20, IConfidentialKRC20Wrapped, ReentrancyGuard, TFHEApp {
    using SafeERC20 for IERC20Metadata;

    /// @notice Returned if the maximum decryption delay is higher than 1 day.
    error MaxDecryptionDelayTooHigh();

    /// @notice KRC20 token that is wrapped.
    IERC20Metadata public immutable KRC20_TOKEN;
    ...
    /**
     * @param krc20_                    Address of the KRC20 token to wrap/unwrap.
     * @dev                             The name/symbol are autogenerated.
     *                                  For instance,
     *                                  "Wrapped KNT" --> "Confidential Wrapped KNT"
     *                                  "WKNT" --> "WKNTc".
     ...
     */
    constructor(address krc20_, uint256 maxDecryptionDelay_)
        ConfidentialKRC20(
            string(abi.encodePacked("Confidential ", IERC20Metadata(krc20_).name())),
            string(abi.encodePacked(IERC20Metadata(krc20_).symbol(), "c"))
        )
    {
        KRC20_TOKEN = IERC20Metadata(krc20_);
    ...
    /**
     * @notice         Unwrap ConfidentialKRC20 tokens to standard KRC20 tokens.
    ...
    /**
     * @notice         Wrap KRC20 tokens to an encrypted format.
     * @param amount   Amount to wrap.
     */
    function wrap(uint256 amount) public virtual {
        KRC20_TOKEN.safeTransferFrom(msg.sender, address(this), amount);

        uint256 amountAdjusted = amount / (10 ** (KRC20_TOKEN.decimals() - decimals()));
    ...
    function callbackUnwrap(uint256 requestId, bool canUnwrap) public virtual nonReentrant onlyGateway {
        ...
        if (canUnwrap) {
            ...
            uint256 amountUint256 = unwrapRequest.amount * (10 ** (KRC20_TOKEN.decimals() - decimals()));

            try KRC20_TOKEN.transfer(unwrapRequest.account, amountUint256) {
