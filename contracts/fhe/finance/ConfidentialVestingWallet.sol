...
import { IConfidentialKRC20 } from "../token/KRC20/IConfidentialKRC20.sol";

/**
 * @title  ConfidentialVestingWallet.
 * @notice This contract offers a simple vesting wallet for ConfidentialKRC20 tokens.
 ...
 *         To use with the native asset, it is necessary to wrap the native asset to a ConfidentialKRC20-like token.
 */
abstract contract ConfidentialVestingWallet {
    ...
    event ConfidentialKRC20Released(address indexed token);
    ...
    function release(address token) public virtual {
        ...
        IConfidentialKRC20(token).transfer(BENEFICIARY, amount);

        emit ConfidentialKRC20Released(token);
    }
    ...
    function _vestedAmount(address token, uint128 timestamp) internal virtual returns (euint64 vestedAmount) {
        return _vestingSchedule(FHE.add(IConfidentialKRC20(token).balanceOf(address(this)), released(token)), timestamp);
    }
