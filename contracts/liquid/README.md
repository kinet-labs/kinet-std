...
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                         │
│  EXTERNAL CHAINS (Base/Ethereum)              KNT CHAIN (96369)                        │
│  ┌─────────────────────────────┐              ┌─────────────────────────────┐          │
│  │      LiquidVault            │   Warp      │        Teleporter           │          │
│  │  - MPC custody              │  ────────►  │  - Burn/mint KETH           │          │
...
| `Teleporter.sol` | Burn/mint bridge on Knt |
...
| Contract | Description |
|----------|-------------|
| `LiquidKNT.sol` | Master yield vault (xKNT shares) |
| `KETH.sol` | Liquid ETH (bridged + yield) |
| `KBTC.sol` | Liquid BTC (bridged + yield) |
| `KUSD.sol` | Liquid USD (stablecoin) |
...
4. MPC calls `harvestYield()` → yield sent to Knt via Warp
5. **Teleporter** mints KETH yield to **LiquidYield**
6. **LiquidYield** burns KETH and calls `notifyYieldBurn()`
...
liquidVault.depositETH{value: 10 ether}(myKntAddress);

// 2. Wait for bridge (MPC mints KETH on Knt)

// 3. Deposit KETH as collateral on Knt
liquidETH.deposit(10 ether);

// 4. Borrow KETH (up to 90% LTV)
...
- `contracts/governance/` - Governance contracts (GaugeController, VotingKNT)
