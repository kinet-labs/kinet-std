 ...
 *
 * Flow:
 * 1. Teleporter mints yield KETH here (from remote strategy harvests)
 * 2. LiquidYield burns the KETH (reducing supply)
 * 3. Notifies LiquidETH to credit debt repayment pro-rata
 *
 * The burn mechanism:
 * - Yield KETH is burned, reducing total KETH supply
...
contract LiquidYield is Ownable, AccessControl, ReentrancyGuard {
    ...
    /// @notice KETH token (burnable)
    IKETHBurnable public immutable leth;
    ...
    /**
     * @notice Called by Teleporter when yield KETH is minted
     * @param amount Amount of yield KETH received
    ...
    /**
     * @notice Process pending yield - burn KETH and notify LiquidETH
...
/**
 * @notice Interface for KETH token with burn
 */
interface IKETHBurnable {
