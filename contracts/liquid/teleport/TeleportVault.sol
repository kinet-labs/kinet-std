 ...
 *
 * Architecture:
 * - Deposits assets on source chain (Base/Ethereum), emits event for Knt bridge
...
abstract contract TeleportVault is Ownable, AccessControl, ReentrancyGuard {
    ...
    struct DepositProof {
        uint256 nonce;
        address kntRecipient;
        uint256 amount;
        uint256 timestamp;
        bool bridged; // Has been bridged to Knt
    }
    ...
    /// @notice Knt C-Chain ID for cross-chain messaging
    uint256 public constant KNT_CHAIN_ID = 96369;
    ...
    event Deposit(uint256 indexed nonce, address indexed kntRecipient, uint256 amount, uint256 srcChainId);

    /// @notice Emitted on asset release for Knt withdrawals
     ...
     * @param kntRecipient Recipient on Knt
     * @param amount Amount deposited
     * @return nonce Unique deposit identifier
     */
    function _recordDeposit(address kntRecipient, uint256 amount) internal returns (uint256 nonce) {
        if (amount == 0) revert ZeroAmount();
        if (kntRecipient == address(0)) revert ZeroAddress();

        nonce = ++depositNonce;

        deposits[nonce] = DepositProof({
            nonce: nonce, kntRecipient: kntRecipient, amount: amount, timestamp: block.timestamp, bridged: false
        });

        totalDeposited += amount;

        emit Deposit(nonce, kntRecipient, amount, block.chainid);
    }
