 ...
 * - Debt: KETH (liquid debt token, minted by vault)
 * - Self-repaying: yield burns debt pro-rata
 * - Min backing: total ETH collateral >= total KETH debt
 */
contract LiquidETH is Ownable, AccessControl, ReentrancyGuard {
    ...
    struct Position {
        ...
        uint256 debt; // KETH debt (borrowed liquid token)
    ...
    /// @notice Liquid token (KETH - minted by this vault)
    ...
    /// @notice Total KETH debt outstanding
     ...
     * @param _liquidToken KETH liquid token address (must be mintable by this vault)
     */
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // BORROW / REPAY (KETH Debt Token)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Borrow KETH against ETH collateral
     * @param amount Amount of KETH to borrow (debt token minted to user)
     */
    function borrow(uint256 amount) external nonReentrant whenNotPaused updateYield(msg.sender) {
        ...
        // Check LTV (ETH collateral vs KETH debt)
    ...
    /**
     * @notice Repay KETH debt
     * @param amount Amount of KETH to repay
     */
    function repay(uint256 amount) external nonReentrant whenNotPaused updateYield(msg.sender) {
        ...
        // Transfer KETH from user and burn it
     ...
     * @param amount Amount of KETH received as yield
    ...
    /**
     * @notice Legacy yield notification (burns KETH to reduce debt)
     * @param amount Amount of KETH burned as yield
     * @dev Called by LiquidYield after burning KETH
     * @dev H-04 fix: Added time-weighted yield index
     */
     ...
     * @param debtToCover Amount of KETH debt to cover
     */
    function liquidate(address user, uint256 debtToCover) external nonReentrant whenNotPaused updateYield(user) {
        ...
        // Transfer KETH payment from liquidator and burn
