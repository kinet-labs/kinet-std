 ...
 *
 * Token Model:
 * - Teleporter mints COLLATERAK tokens (e.g., bridged ETH)
 * - Collateral must be deposited into LiquidETH vault to get L* tokens
 * - L* tokens (e.g., KETH) are the yield-bearing liquid tokens
 *
 * Flow:
 * 1. User deposits ETH on Ethereum/Base → LiquidVault
 * 2. MPC attests → Teleporter.mintDeposit() → mints ETH to user on Knt
 * 3. User deposits ETH into LiquidETH → receives KETH (yield-bearing)
 * 4. Yield harvested → mintYield() → KETH minted to LiquidYield → reduces debt
 *
 * Domain Separation:
...
contract Teleporter is Ownable, AccessControl, ReentrancyGuard {
    ...
    /// @notice The liquid token (KETH, KUSD, etc.) - minted for yield
     ...
     * @param _collateralToken Bridged collateral token (e.g., ETH on Knt)
     * @param _liquidToken Liquid token (e.g., KETH)
     ...
     * @dev Mints the COLLATERAK token (e.g., bridged ETH), NOT the liquid token
     ...
     * @param recipient Collateral recipient on Knt
     ...
     * @dev Mints the LIQUID token (e.g., KETH) to LiquidVault for debt repayment
     ...
     * @param _liquidVault LiquidVault address on Knt
     */
