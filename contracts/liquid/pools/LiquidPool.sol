 ...
 *         normalization), mints the basket's K token; on burn, the user
 ...
 *   - the K token address (already deployed; pool must hold mint+burn admin)
...
abstract contract LiquidPool is AccessControl, ReentrancyGuard, Pausable, PerAssetLedger, SolvencyStateMachineV4 {
    ...
    /// @notice The K token this pool issues (KUSD/KBTC/KETH/…)
    IBridgedToken public immutable liquidToken;

    /// @notice The pool's base-unit decimals (KUSD/KETH=18, KBTC=8, KSOL/KTON=9, KXRP=6, KDOT=10)
    ...
    /// @param _liquidToken     K token address; the pool must already be granted
    ///                         DEFAULT_ADMIN_ROLE / MINTER_ROLE on it
    /// @param _basketRegistry  basket registry (KUSD pool reads BasketClass.USD, etc.)
    ...
    constructor(
        ...
        // Pool reserves are 1:1 with the K token supply by construction
    ...
    // ─────────────────────────────────────────────────────────────────────
    //  DEPOSIT (any basket member → K token at 1:1, decimal-normalized)
    // ─────────────────────────────────────────────────────────────────────

    /**
     * @notice Deposit a registered basket asset and receive the K token.
    ...
    /**
     * @notice Burn K tokens; receive `preferredAsset` from the basket inventory.
        ...
        returns (uint256 rawOut)
    {
        ...
        // Burn-with-allowance: caller must approve pool to spend their K tokens.
