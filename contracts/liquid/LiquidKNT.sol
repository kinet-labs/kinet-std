...
/**
 * @title LiquidKNT (xKNT)
 * @author kinet labs
 * @notice Master yield vault that receives ALL protocol fees and mints xKNT shares
 *
 * ARCHITECTURE:
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │                           LiquidKNT (xKNT)                                  │
 ...
 * │                                                                             │
 * │  OUTFLOWS:                                                                  │
 * │  • Users deposit KNT → receive xKNT shares                                 │
 * │  • Users withdraw xKNT → receive KNT + proportional yield                  │
 * │  • xKNT is checkpointed (ERC20Votes) for flash-loan-resistant governance   │
 * │                                                                             │
 * │  GOVERNANCE:                                                                │
 * │  • vKNT = xKNT + DKNT (aggregated in VotingKNT contract)                   │
 ...
 */
contract LiquidKNT is ERC20, ERC20Permit, ERC20Votes, ReentrancyGuard, AccessControl, Pausable {
    ...
    constructor(address _knt, address _treasury, address _timelock)
        ERC20("Liquid KNT", "xKNT")
        ERC20Permit("Liquid KNT")
    {
        if (_knt == address(0) || _treasury == address(0)) revert InvalidAddress();
    ...
    /**
     * @notice Deposit KNT and receive xKNT shares
     * @param amount Amount of KNT to deposit
     * @return shares Amount of xKNT shares minted
     */
    function deposit(uint256 amount) external nonReentrant whenNotPaused returns (uint256 shares) {
        if (amount == 0) revert ZeroAmount();

        // Transfer KNT from user first (prevents reentrancy manipulation of totalAssets)
    ...
    /**
     * @notice Withdraw KNT by burning xKNT shares
     * @param shares Amount of xKNT shares to burn
     * @return amount Amount of KNT withdrawn
     */
     ...
     * @param amount Amount of KNT fees
    ...
    function receiveFees(uint256 amount, bytes32 feeType) external nonReentrant whenNotPaused {
        ...
        if (perfFee > 0) {
            knt.safeTransfer(treasury, perfFee);
        }
     ...
     * @param amount Amount of KNT rewards
     */
    function depositValidatorRewards(uint256 amount) external nonReentrant whenNotPaused {
        ...
        // Transfer KNT from validator source
        knt.safeTransferFrom(msg.sender, address(this), amount);
     ...
     * @param amount Amount of KNT lost to slashing
     */
    function applySlashing(uint256 amount) external onlyRole(GOVERNANCE_ROLE) {
        ...
        if (slashingReserve >= amount) {
            ...
            if (socializeLosses) {
                ...
                // This effectively dilutes all xKNT holders proportionally
    ...
    function emergencyWithdrawToTreasury() external onlyRole(EMERGENCY_ROLE) {
        ...
        knt.safeTransfer(treasury, balance);
    ...
    /**
     * @notice Total KNT assets in the vault
     */
    function totalAssets() public view returns (uint256) {
        return knt.balanceOf(address(this));
    }

    /**
     * @notice Convert KNT amount to xKNT shares
     */
    ...
    /**
     * @notice Convert xKNT shares to KNT amount
     */
    ...
    /**
     * @notice Current exchange rate (KNT per xKNT, scaled by 1e18)
     */
     ...
     * @return actualBalance Actual KNT balance
    ...
    function reconcile() external view returns (uint256 expectedBalance, uint256 actualBalance, int256 discrepancy) {
        ...
        actualBalance = knt.balanceOf(address(this));
