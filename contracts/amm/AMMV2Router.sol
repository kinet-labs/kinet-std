...
contract AMMV2Router is ReentrancyGuard {
    ...
    constructor(address _factory, address _wknt) {
        factory = _factory;
        WKNT = _wknt;
    }

    receive() external payable {
        require(msg.sender == WKNT, "AMMV2Router: NOT_WKNT");
    }
    ...
    function addLiquidityKNT(
        address token,
        uint256 amountTokenDesired,
        uint256 amountTokenMin,
        uint256 amountKNTMin,
        address to,
        uint256 deadline
    )
        external
        ...
        nonReentrant
        returns (uint256 amountToken, uint256 amountKNT, uint256 liquidity)
    {
        (amountToken, amountKNT) = _addLiquidity(
            token, WKNT, amountTokenDesired, msg.value, amountTokenMin, amountKNTMin
        );
        address pair = AMMV2Factory(factory).getPair(token, WKNT);
        IERC20(token).safeTransferFrom(msg.sender, pair, amountToken);
        IWKNT(WKNT).deposit{ value: amountKNT }();
        IERC20(WKNT).safeTransfer(pair, amountKNT);
        liquidity = AMMV2Pair(pair).mint(to);
        // Refund excess KNT
        if (msg.value > amountKNT) {
            (bool success,) = msg.sender.call{ value: msg.value - amountKNT }("");
            require(success, "AMMV2Router: KNT_REFUND_FAILED");
        }
    }
    ...
    function removeLiquidityKNT(
        ...
        address to,
        uint256 deadline
    ) public ensure(deadline) nonReentrant returns (uint256 amountToken, uint256 amountKNT) {
        (amountToken, amountKNT) = _removeLiquidity(token, WKNT, liquidity, amountTokenMin, amountKNTMin, address(this));
        IERC20(token).safeTransfer(to, amountToken);
        IWKNT(WKNT).withdraw(amountKNT);
        (bool success,) = to.call{ value: amountKNT }("");
        require(success, "AMMV2Router: KNT_TRANSFER_FAILED");
    }
    ...
    function swapExactKNTForTokens(uint256 amountOutMin, address[] calldata path, address to, uint256 deadline)
        external
        payable
        ensure(deadline)
        nonReentrant
    ...
    {
        require(path[0] == WKNT, "AMMV2Router: INVALID_PATH");
        amounts = getAmountsOut(msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "AMMV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        IWKNT(WKNT).deposit{ value: amounts[0] }();
        IERC20(WKNT).safeTransfer(AMMV2Factory(factory).getPair(path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }

    function swapExactTokensForKNT(
        uint256 amountIn,
        uint256 amountOutMin,
        address[] calldata path,
        address to,
        ...
        require(path[path.length - 1] == WKNT, "AMMV2Router: INVALID_PATH");
        amounts = getAmountsOut(amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "AMMV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        IERC20(path[0]).safeTransferFrom(msg.sender, AMMV2Factory(factory).getPair(path[0], path[1]), amounts[0]);
        _swap(amounts, path, address(this));
        IWKNT(WKNT).withdraw(amounts[amounts.length - 1]);
