 ...
 *     - KUSD as primary settlement currency
...
import { IKRC20 } from "../tokens/interfaces/IKRC20.sol";
...
contract Market is IMarket, Ownable, ReentrancyGuard {
    ...
    struct Listing {
        ...
        address nftContract;
        uint256 tokenId;
        address paymentToken; // address(0) = native KNT, otherwise KRC20
        uint256 price;
        uint256 expiration;
        bool active;
    }
    ...
    /// @notice KUSD stablecoin (primary payment token)
    IKRC20 public lusd;
     ...
     * @param paymentToken Payment token (address(0) for native KNT)
    ...
    function buy(bytes32 listingId) external payable whenNotPaused nonReentrant {
        ...
        if (listing.paymentToken == address(0)) {
            ...
            // KRC20 payment
            IKRC20 token = IKRC20(listing.paymentToken);
     ...
     * @param paymentToken Payment token (must be KRC20, not native)
        ...
        returns (bytes32 offerId)
    {
        ...
        if (duration == 0) revert InvalidExpiration();
        if (paymentToken == address(0)) revert InvalidPrice(); // Must use KRC20 for offers

        // Verify buyer has funds and approval
        IKRC20 token = IKRC20(paymentToken);
    ...
    function acceptOffer(bytes32 offerId) external whenNotPaused nonReentrant {
        ...
        // Handle payment (always KRC20 for offers)
        IKRC20 token = IKRC20(offer.paymentToken);
