...
// state (they're brand-neutral KRC20B wrappers), so they're safe to
 ...
 * @dev Usage:
 *
 *   TENANT_FILE=script/tenants/knt.json \
 *   KNT_PRIVATE_KEY=0x... \
...
contract DeployTenant is Script {
    ...
    function _basketClassOf(string memory symbol) private pure returns (BasketRegistry.BasketClass) {
        ...
        if (h == keccak256("KNT")) return BasketRegistry.BasketClass.KNT;
    ...
    function run() external {
        ...
        // Baskets are USD/BTC/ETH/SOL/TON/XRP/DOT (KNT is native, reserved).
