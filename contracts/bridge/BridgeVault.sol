 ...
 *     ╚═════╝ ╚═╝  ╚═╝╚═╝╚═════╝  ╚═════╝ ╚══════╝      ╚═══╝  ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝
 *
 *     Unified Bridge Vault - Used by Knt, and all ecosystem chains
 */

import { KRC4626 } from "../tokens/KRC4626/KRC4626.sol";
...
/**
 * @title BridgeVault
 * @notice Unified vault for all Knt ecosystem bridge operations
 * @dev Manages ERC20 and ETH vaults for cross-chain bridging
 * Used by: Knt mainnet and all ecosystem chains
 */
contract BridgeVault is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Add new ERC20 vault using KRC4626
     * @param asset_ ERC20 token address
     */
    function _addNewERC20Vault(address asset_) private {
        require(erc20Vault[asset_] == address(0), "Vault already exists.");
        KRC4626 newERC20Vault =
            new KRC4626(IERC20(asset_), concat(KRC20(asset_).name(), " Vault"), concat("v", KRC20(asset_).symbol()));
    ...
    function deposit(address asset_, uint256 amount_) external payable onlyOwner nonReentrant {
        if (asset_ == address(0)) {
            ...
            KRC4626(erc20Vault[asset_]).deposit(amount_, owner());
        }
    }
    ...
    function withdraw(address asset_, address receiver_, uint256 amount_) external onlyOwner nonReentrant {
        if (asset_ == address(0)) {
            ...
            KRC4626(erc20Vault[asset_]).withdraw(amount_, receiver_, owner());
        }
    }
    ...
    function previewWithdraw(address asset_) public view returns (uint256) {
        if (asset_ == address(0)) {
            ...
            if (erc20Vault[asset_] == address(0)) {
                ...
                return KRC4626(erc20Vault[asset_]).maxWithdraw(owner());
            }
        }
    }
        ...
        )
    {
        if (asset_ == address(0)) {
            ...
            return (
                KRC20(asset_).name(),
                KRC20(asset_).symbol(),
                KRC4626(erc20Vault[asset_]).name(),
                KRC4626(erc20Vault[asset_]).symbol(),
                erc20Vault[asset_],
                KRC4626(erc20Vault[asset_]).totalAssets()
            );
        }
    }
