 ...
 *
 * Architecture:
 * - Deposits ETH on Base/Ethereum, emits event for Knt bridge
 * - MPC controls withdrawals and strategy allocations
 * - Strategies earn yield that flows to Knt for debt repayment
...
contract LiquidVault is TeleportVault, Pausable {
    ...
    /**
     * @notice Deposit ETH to be bridged to Knt
     * @param kntRecipient Address to receive KETH on Knt
     * @return nonce Unique deposit identifier
     */
    function depositETH(address kntRecipient) external payable nonReentrant returns (uint256 nonce) {
        nonce = _recordDeposit(kntRecipient, msg.value);
    }

    /**
     * @notice Deposit ETH for self (msg.sender receives on Knt)
     * @return nonce Unique deposit identifier
     */
    ...
    /**
     * @notice Release ETH for Knt withdrawal (MPC only)
