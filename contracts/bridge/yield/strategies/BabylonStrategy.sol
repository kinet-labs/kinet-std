 ...
 *
 * 2. LombardKBTCStrategy - Lombard KBTC liquid staking
 *    - Liquid staking token wrapping Babylon-staked BTC
 *    - No lockup, instant liquidity via KBTC token
...
/// @notice Lombard KBTC liquid staking token interface
interface IKBTC {
    /// @notice Deposit BTC to receive KBTC
    /// @param btcAmount Amount of BTC to deposit
    /// @return lbtcAmount Amount of KBTC received
    function deposit(uint256 btcAmount) external returns (uint256 lbtcAmount);

    /// @notice Withdraw BTC by burning KBTC
    /// @param lbtcAmount Amount of KBTC to burn
    /// @return btcAmount Amount of BTC received
    function withdraw(uint256 lbtcAmount) external returns (uint256 btcAmount);

    /// @notice Get current exchange rate (BTC per KBTC, scaled by 1e18)
...
interface ILombardStaking {
    ...
    /// @notice Get staked KBTC balance
    /// @param account Staker address
...
contract BabylonBTCStrategy is Ownable {
    ...
    /// @notice WBTC or KBTC token used as deposit
...
// ═══════════════════════════════════════════════════════════════════════════════
// LOMBARD KBTC STRATEGY
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * @title LombardKBTCStrategy
 * @notice Liquid BTC staking via Lombard KBTC
 * @dev Wraps BTC into KBTC liquid staking token with optional Lombard staking
 *
 * Yield Sources:
 * 1. Base KBTC yield from Babylon staking (auto-compounding)
 ...
 *
 * Trade-offs:
 * - Smart contract risk (KBTC contracts)
 * - Slight yield reduction for liquidity premium
 * - Exchange rate risk (KBTC:BTC ratio)
 */
contract LombardKBTCStrategy is Ownable {
    using SafeERC20 for IERC20;
    ...
    /// @notice Lombard KBTC token
    IKBTC public immutable lbtc;
    ...
    /// @notice KBTC balance held (either directly or staked)
    uint256 public totalKBTC;
    ...
    /// @notice Emitted when BTC is deposited for KBTC
    event Deposited(uint256 btcAmount, uint256 lbtcReceived, bool staked);

    /// @notice Emitted when KBTC is withdrawn for BTC
    ...
    /**
     * @notice Construct Lombard KBTC strategy
     * @param _vault Vault that controls this strategy
     * @param _lbtc KBTC token address
    ...
    constructor(address _vault, address _lbtc, address _btcToken, address _lombardStaking) Ownable(msg.sender) {
        ...
        // Approve KBTC to spend BTC for deposits
        btcToken.approve(_lbtc, type(uint256).max);

        // Approve Lombard staking to spend KBTC if enabled
    ...
    function deposit(uint256 amount) external onlyVault whenActive returns (uint256 shares) {
        ...
        // Deposit BTC for KBTC
        uint256 lbtcReceived = lbtc.deposit(amount);
        totalKBTC += lbtcReceived;
    ...
    function harvest() external returns (uint256 harvested) {
        ...
        if (currentRate > lastExchangeRate && totalKBTC > 0) {
            // Yield = KBTC * (new_rate - old_rate) / PRECISION
            uint256 rateGain = currentRate - lastExchangeRate;
            uint256 yieldInBTC = (totalKBTC * rateGain) / PRECISION;
    ...
    /// @notice
    function totalAssets() external view returns (uint256) {
        // KBTC value in BTC terms
        uint256 lbtcValue = (totalKBTC * lbtc.exchangeRate()) / PRECISION;
    ...
    /// @notice
    function currentAPY() external view returns (uint256) {
        // Base KBTC yield (from Babylon staking)
    ...
    /// @notice
    function name() external pure returns (string memory) {
        return "Lombard KBTC Liquid Staking Strategy";
    }
    ...
    /// @notice Get current KBTC exchange rate
    function exchangeRate() external view returns (uint256) {
    ...
    /// @notice Get KBTC balance breakdown
...
contract BabylonStrategyFactory {
    ...
    /**
     * @notice Deploy a Lombard KBTC liquid staking strategy
     * @param vault Vault address
     * @param lbtc KBTC token address
     * @param btcToken WBTC token address
    ...
    function deployLombard(address vault, address lbtc, address btcToken, address lombardStaking)
        ...
        returns (address strategy)
    {
        strategy = address(new LombardKBTCStrategy(vault, lbtc, btcToken, lombardStaking));
