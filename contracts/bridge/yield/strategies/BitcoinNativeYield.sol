 ...
 *
 *      2. Lombard Finance (KBTC liquid staking)
 *         - Deposit BTC → receive KBTC (liquid staking token)
 *         - KBTC earns Babylon + Lombard rewards
 *         - ~4-6% APY
 *
 ...
 *      - User receives yKBTC on Knt (yield-bearing bridged BTC)
 *      - yKBTC earns aggregated BTC yield from all strategies
 *      - yKBTC usable in LPX Perps, Markets, AMMs on Knt
 *      - MPC reports yield back via Warp → yKBTC share price increases
 */
...
interface ILombard {
    ...
    function exchangeRate() external view returns (uint256); // KBTC per BTC (scaled 1e18)
...
contract BitcoinNativeYield is IYieldStrategy {
    ...
    struct StrategyAllocation {
        ...
        uint256 lombardBps; // Lombard KBTC
