 ...
 * - Liquity: KUSD stability pool + LQTY staking, ETH liquidation gains
...
/// @notice Liquity Stability Pool - deposit KUSD, earn ETH + LQTY
interface IStabilityPool {
    /// @notice Provide KUSD to stability pool
    function provideToSP(uint256 _amount) external;

    /// @notice Withdraw KUSD from stability pool
    ...
    /// @notice Get depositor's compounded KUSD deposit
    ...
    /// @notice Total KUSD deposits
    function getTotalKUSDDeposits() external view returns (uint256);
}

/// @notice LQTY staking - stake LQTY, earn ETH + KUSD from fees
interface ILQTYStaking {
    ...
    /// @notice Get pending KUSD gains from borrowing fees
 ...
 * @notice Yield strategy for Liquity KUSD Stability Pool
 * @dev Deposits KUSD to earn ETH + LQTY from liquidations
 *
 * Yield sources:
 * - ETH gains from liquidations (~5-20% APY variable)
 * - LQTY rewards (~2-5% APY)
 * - Optional LQTY staking for ETH + KUSD fees
 *
 * Key features:
...
contract LiquityStrategy is Ownable {
    ...
    /// @notice KUSD stablecoin
    address public constant KUSD = 0x5f98805A4E8be255a32880FDeC7F6728C6568bA0;
    ...
    /// @notice Total KUSD deposited (before compounding)
    uint256 public totalDeposited;
    ...
    constructor(address _vault) Ownable(msg.sender) {
        ...
        // Approve stability pool to spend KUSD
        IERC20(KUSD).approve(STABILITY_POOL, type(uint256).max);
        ...
        returns (uint256 shares)
    {
        ...
        // Transfer KUSD from vault
        IERC20(KUSD).safeTransferFrom(msg.sender, address(this), amount);

        // Deposit to Stability Pool
        stabilityPool.provideToSP(amount);
        totalDeposited += amount;

        // Shares are 1:1 with KUSD for simplicity
        shares = amount;
    ...
    function withdraw(
        ...
        returns (uint256 amount)
    {
        uint256 compounded = stabilityPool.getCompoundedKUSDDeposit(address(this));
        ...
        // Transfer KUSD to recipient
        amount = shares;
        IERC20(KUSD).safeTransfer(vault, amount);
    ...
    function harvest() external returns (uint256 harvested) {
        ...
        if (stakedLqty > 0) {
            ...
            uint256 pendingLusd = lqtyStaking.getPendingKUSDGain(address(this));
            ...
            if (pendingEth > 0 || pendingLusd > 0) {
                ...
                uint256 lusdFromStaking = IERC20(KUSD).balanceOf(address(this));
                if (lusdFromStaking > 0) {
                    IERC20(KUSD).safeTransfer(vault, lusdFromStaking);
                }
    ...
    /// @notice
    function totalAssets() external view returns (uint256) {
        return stabilityPool.getCompoundedKUSDDeposit(address(this));
    }
    ...
    /// @notice Get yield token address
    function yieldToken() external pure returns (address) {
        // No separate yield token - KUSD in stability pool
        return address(0);
    }
    ...
    /// @notice Get compounded KUSD balance
    ...
    function pendingStakingRewards() external view returns (uint256 eth, uint256 lusd) {
        ...
        lusd = lqtyStaking.getPendingKUSDGain(address(this));
    }
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        uint256 deposited = stabilityPool.getCompoundedKUSDDeposit(address(this));
        if (deposited > 0) {
            stabilityPool.withdrawFromSP(deposited);
        }

        // Transfer all tokens
        uint256 lusdBalance = IERC20(KUSD).balanceOf(address(this));
        if (lusdBalance > 0) {
            IERC20(KUSD).safeTransfer(owner(), lusdBalance);
        }
    ...
    /// @notice Rescue stuck tokens
    function rescueToken(address token, uint256 amount) external onlyOwner {
        require(token != KUSD, "Cannot rescue KUSD");
        IERC20(token).safeTransfer(owner(), amount);
    }
