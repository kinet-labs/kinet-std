 ...
 * @notice KRC20 Bridge Token - Base contract for bridged tokens
 * @dev Extends KRC20 with bridge mint/burn capabilities and role-based access
 */
contract KRC20B is KRC20, Ownable, AccessControl {
    // ═══════════════════════════════════════════════════════════════════════
    ...
    constructor(string memory name_, string memory symbol_) KRC20(name_, symbol_) Ownable(msg.sender) {
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "KRC20B: caller is not admin");
        _;
    }
    ...
    function revokeAdmin(address to) public onlyAdmin {
        require(hasRole(DEFAULT_ADMIN_ROLE, to), "KRC20B: not an admin");
    ...
    function bridgeMint(address account, uint256 amount) public returns (bool) {
        require(hasRole(MINTER_ROLE, msg.sender), "KRC20B: caller is not minter");
    ...
    /**
     * @dev Override _msgSender for OZ AccessControl/Ownable and KRC20 compatibility
     */
