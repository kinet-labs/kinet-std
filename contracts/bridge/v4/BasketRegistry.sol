 ...
 *   USD basket  → LiquidUSD (KUSD)
 *   BTC basket  → LiquidBTC (KBTC)
 *   ETH basket  → LiquidETH (KETH)
 *   SOL basket  → LiquidSOL (KSOL)
 *   TON basket  → LiquidTON (KTON)
 *   XRP basket  → LiquidXRP (KXRP)
 *   DOT basket  → LiquidDOT (KDOT)
 *   KNT basket  → KNT/sKNT (native, reserved)
 *
...
contract BasketRegistry is AccessControl {
    ...
    /// @param asset          bridged-asset contract address (KRC20B descendant)
