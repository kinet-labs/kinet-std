 ...
 * @dev Routes governance actions between Knt (home) and teleport locales (Base, etc.)
 *
 * Architecture:
 * - Knt C-Chain: Primary governance hub
 * - Knt T-Chain: FHE private voting
 * - Knt Z-Chain: ZK-UTXO shielded treasury
 * - Base/Others: Teleport locales for asset bridging/listing
 *
 * Message Types:
 * - PROPOSAL_CREATE: New proposal from teleport locale → Knt
...
contract TeleportProposalBridge is AccessControl, ReentrancyGuard {
    ...
    uint256 public constant KNT_C_CHAIN = 96369;
    uint256 public constant KNT_T_CHAIN = 96370;
    uint256 public constant KNT_Z_CHAIN = 96371;
    ...
    function _initializeKntLocales() internal {
        // Register Knt chains as locales
        _registerLocale(KNT_C_CHAIN, "Knt C-Chain", address(0));
        _registerLocale(KNT_T_CHAIN, "Knt T-Chain (TFHE)", address(0));
        _registerLocale(KNT_Z_CHAIN, "Knt Z-Chain (zkVM)", address(0));
     ...
     * @param destChainId Destination chain (typically Knt C-Chain)
        ...
        returns (bytes32 messageId)
    {
        ...
        messageId = _queueMessage(KNT_T_CHAIN, MessageType.VOTE_BATCH, payload);
    ...
    function queueTallyResult(
        ...
        messageId = _queueMessage(KNT_C_CHAIN, MessageType.TALLY_RESULT, payload);
        ...
        returns (bytes32 messageId)
    {
        ...
        messageId = _queueMessage(KNT_Z_CHAIN, MessageType.SPEND_AUTH, payload);
    ...
    function deactivateLocale(uint256 chainId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (!locales[chainId].active) revert LocaleNotFound();
        // Cannot deactivate Knt chains
        require(
            chainId != KNT_C_CHAIN && chainId != KNT_T_CHAIN && chainId != KNT_Z_CHAIN, "Cannot deactivate Knt chains"
        );
