 ...
 * @title Registry
 * @author kinet labs
 * @notice Multi-chain DID Registry for the Knt ecosystem
 * @dev Manages decentralized identities across Knt, Pars chains
 *
 * DID Formats:
 *   did:knt:alice   - W3C DID method (canonical, for protocols)
 *   alice[at]knt.id - Display format (user-friendly, like email)
 *
 * Supported Networks:
 * - Knt:   did:knt:alice   / alice[at]knt.id
...
contract Registry is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, ReentrancyGuard {
    ...
    /// @notice Staking token (ASHA, etc.)
    IERC20 public stakingToken;

    /// @notice Identity NFT contract
    IIdentityNFT public identityNft;

    /// @notice Chain ID to DID method (e.g., 96369 => "knt")
    mapping(uint256 => string) public methods;

    /// @notice Chain ID to display domain (e.g., 96369 => "knt.id")
    ...
    function initialize(address owner_, address stakingToken_, address identityNft_) public initializer {
        ...
        // Knt ecosystem
        methods[96369] = "knt";
        domains[96369] = "knt.id";
        methods[96368] = "knt-test";
        domains[96368] = "test.knt.id";
    ...
    function claim(ClaimParams calldata params, bytes32 secret) external nonReentrant returns (string memory did) {
        ...
        // Construct canonical DID: did:knt:alice
    ...
    /**
     * @notice Get canonical DID format: did:knt:alice
     */
    ...
    /**
     * @notice Get display format: alice@knt.id
     */
