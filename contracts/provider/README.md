...
Knt exchange is jurisdiction-neutral OSS. Some forks need to offer
...
- **No provider configured** (`address(0)` or `NullProvider`): every swap
  goes through native Knt AMM pools. Pure crypto-to-crypto DeFi.
...
- **Clean separation**: Knt repo stays OSS / permissionless / universal.
...
- **Regulator-friendly**: compliance state lives in the provider's own
  repo, reviewed by that provider's own regulators — not Knt core.
