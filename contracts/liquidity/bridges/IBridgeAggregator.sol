...
/// @dev Supports Axelar GMP, LayerZero V2, Wormhole CCTP, and Knt Warp
interface IBridgeAggregator {
    ...
    enum BridgeProtocol {
        KNT_WARP, // Native Knt Warp messaging (fastest for Knt ecosystem)
...
/// @title IWarp
/// @notice Knt Warp native cross-chain messaging
interface IWarp {
...
library BridgeLib {
    /// @notice Knt Warp precompile address
    ...
    function getLzEndpointId(uint256 chainId) internal pure returns (uint32) {
        ...
        if (chainId == 96369) return 30369; // Knt (custom EID)
    ...
    function getWormholeChainId(uint256 chainId) internal pure returns (uint16) {
        ...
        if (chainId == 96369) return 36; // Knt (reserved)
...
/// @title WarpReceiver
/// @notice Base contract for Knt Warp message receivers
