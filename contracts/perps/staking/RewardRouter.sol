...
contract RewardRouter is IRewardRouter, ReentrancyGuard, Governable {
    ...
    address public lpx;
    address public xLPX;
    address public dknt; // DKNT governance token - single reward token

    address public llp; // LLP - Knt Liquidity Provider token
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        ...
        uint256 stakedDKNT = IRewardTracker(feeLPXTracker).depositBalances(_sender, dknt);
        if (stakedDKNT > 0) {
            IRewardTracker(feeLPXTracker).unstakeForAccount(_sender, dknt, stakedDKNT, _sender);
            IRewardTracker(feeLPXTracker).stakeForAccount(_sender, receiver, dknt, stakedDKNT);
        }
        ...
        uint256 dkntBalance = IERC20(dknt).balanceOf(_sender);
        if (dkntBalance > 0) {
            IMintable(dknt).burn(_sender, dkntBalance);
            IMintable(dknt).mint(receiver, dkntBalance);
        }
    ...
    // call _stakeDKNT it should be ensured that _syncVotingPower is called after
    function _stakeDKNT(address _account) private {
        IRewardTracker(bonusLPXTracker).claimForAccount(_account, _account);

        // get the DKNT balance of the user, this would be the amount of
        // DKNT that has not been staked
        uint256 dkntAmount = IERC20(dknt).balanceOf(_account);
        ...
        uint256 maxAllowedDKNTAmount = baseStakedAmount * maxBoostBasisPoints / BASIS_POINTS_DIVISOR;
        uint256 currentDKNTAmount = IRewardTracker(feeLPXTracker).depositBalances(_account, dknt);
        if (currentDKNTAmount == maxAllowedDKNTAmount) return;

        // if the currentDKNTAmount is more than the maxAllowedDKNTAmount
        // unstake the excess tokens
        if (currentDKNTAmount > maxAllowedDKNTAmount) {
            uint256 amountToUnstake = currentDKNTAmount - maxAllowedDKNTAmount;
            IRewardTracker(feeLPXTracker).unstakeForAccount(_account, dknt, amountToUnstake, _account);
            return;
        }

        uint256 maxStakeableDKNTAmount = maxAllowedDKNTAmount - currentDKNTAmount;
        if (dkntAmount > maxStakeableDKNTAmount) {
            dkntAmount = maxStakeableDKNTAmount;
        }

        IRewardTracker(feeLPXTracker).stakeForAccount(_account, _account, dknt, dkntAmount);
    }

    function _unstakeLPX(address _account, address _token, uint256 _amount, bool _shouldReduceDKNT) private {
        require(_amount > 0, "invalid _amount");
        ...
        if (_shouldReduceDKNT) {
            IRewardTracker(bonusLPXTracker).claimForAccount(_account, _account);

            // unstake and burn staked DKNT tokens
            uint256 stakedDKNT = IRewardTracker(feeLPXTracker).depositBalances(_account, dknt);
            if (stakedDKNT > 0) {
                uint256 reductionAmount = stakedDKNT * _amount / balance;
                IRewardTracker(feeLPXTracker).unstakeForAccount(_account, dknt, reductionAmount, _account);
                IMintable(dknt).burn(_account, reductionAmount);
            }

            // burn DKNT tokens from user's balance
            uint256 dkntBalance = IERC20(dknt).balanceOf(_account);
            if (dkntBalance > 0) {
                uint256 amountToBurn = dkntBalance * _amount / balance;
                IMintable(dknt).burn(_account, amountToBurn);
            }
        }
