...
/// @title VesterCap
/// @notice Manages DKNT (governance token) staking caps and conversions
...
contract VesterCap is ReentrancyGuard, Governable {
    ...
    address public immutable dknt; // DKNT governance token
    address public immutable xLPX; // Escrowed LPX

    uint256 public immutable maxBoostBasisPoints;
    uint256 public immutable dkntToXLPXConversionDivisor;
    ...
    constructor(
        ...
        uint256 _dkntToXLPXConversionDivisor
    ) public {
        ...
        dkntToXLPXConversionDivisor = _dkntToXLPXConversionDivisor;
    }

    function updateDKNTForAccounts(address[] memory _accounts) external nonReentrant onlyGov {
        for (uint256 i; i < _accounts.length; i++) {
            _updateDKNTForAccount(_accounts[i]);
        }
    }
    ...
    function _updateDKNTForAccount(address _account) internal {
        ...
        uint256 stakedDKNTAmount = IRewardTracker(feeLPXTracker).depositBalances(_account, dknt);
        uint256 claimableDKNTAmount = IRewardTracker(bonusLPXTracker).claimable(_account);
        uint256 dkntBalance = IERC20(dknt).balanceOf(_account);
        uint256 totalDKNTAmount = stakedDKNTAmount + claimableDKNTAmount + dkntBalance;

        uint256 xLPXToMint = totalDKNTAmount / dkntToXLPXConversionDivisor;
        ...
        if (stakedDKNTAmount <= maxAllowedDKNTAmount) {
            return;
        }

        uint256 amountToUnstake = stakedDKNTAmount - maxAllowedDKNTAmount;
        uint256 feeLPXTrackerBalance = IERC20(feeLPXTracker).balanceOf(_account);
