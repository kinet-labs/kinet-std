...
library Offerings {
    ...
    bytes32 internal constant KNT_RAIF              = keccak256("KNT_RAIF");               // QUALIFIED_PRIVATE (well-informed)
    bytes32 internal constant KNT_SIF               = keccak256("KNT_SIF");                // INSTITUTIONAL_ONLY
    ...
    function normalise(bytes32 offering) internal pure returns (bytes32) {
        ...
        if (offering == UK_FSMA_S86_QI || offering == EU_PR_ART1_4_QI || offering == KNT_RAIF) return QUALIFIED_PRIVATE;
        ...
        if (offering == KNT_SIF) return INSTITUTIONAL_ONLY;
        // Singapore
