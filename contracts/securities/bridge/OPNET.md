...
is treated as just another destination chain by the Knt securities stack. No
new processes are needed — every link is a native Knt chain.

```
┌─ EVM L1 (Knt C-Chain, … any chain ID) ───────────────────────────┐
│                                                                  │
...
└────────┼─────────────────────────────────────────────────────────┘
         │
         ▼
┌─ R-Chain  (kinet/node — chains/relayvm) ──────────────────────────┐
│                                                                    │
...
└────────┼───────────────────────────────────────────────────────────┘
         │
         ▼
┌─ O-Chain  (kinet/node — chains/oraclevm) ────────────────────────┐
│                                                                   │
│   Feed("opnet:btc-tip"):                                          │
│     - sources: [bitcoin RPC, esplora, kinet/indexer + opnet]      │
...
└────────┼──────────────────────────────────────────────────────────┘
         │
         ▼
┌─ MPC (kinet/mpc) — FROST/Taproot threshold ───────────────────────┐
│                                                                    │
...
|---|---|---|---|
| EVM bridge | `@kinet/contracts` | `securities/bridge/SecuritiesGateway.sol` | `outbound(...)`, MPC-signed `inbound(...)` |
| EVM securities | `@kinet/contracts` | `securities/token/SecurityToken.sol` | T-REX `IToken`, `mint` checks `IIdentityRegistry.isVerified` |
| Cross-chain message bus | `kinet/node` | `chains/relayvm` | Channel + SendMessage + GetVerifiedMessage |
| External-data oracle | `kinet/node` | `chains/oraclevm` | Feed + Observation + Aggregation + ZK proof |
| Threshold signer | `kinet/mpc` | `pkg/eventconsumer/signing_handler_frost.go` | FROST/Taproot/EdDSA signing |
| Settlement intent tracker | `kinet/mpc` | `pkg/settlement/intent.go` | Generic chain-agnostic intent, on-chain receipt verification |
| Securities indexing | `kinet/graph` | `indexer/indexer_securities.go`, `resolvers/securities/` | Full ERC-3643 + ONCHAINID event ingestion + GraphQL |
| Generic block explorer | `kinet/indexer` | `multichain/manager.go` | Protocol consts: `erc3643`, `onchainid`, `opnet` |
| OP_NET locale registration | `@kinet/contracts` | `script/RegisterOPNET.s.sol` | Adds OP_NET (chain `4294967299`) to TeleportProposalBridge |
| OP_NET yield strategy | `@kinet/contracts` | `bridge/yield/strategies/OPNETStrategy.sol` | Wraps Babylon BTC staking for bridged KBTC on Knt |
| OP_NET conformance tests | `@kinet/contracts` | `test/teleport/OPNET{Conformance,Outbound}.t.sol` | 36 + 2 tests |
| Securities ↔ OP_NET tests | `@kinet/contracts` | `test/securities/SecuritiesGateway.t.sol` | 7 tests, full round-trip |

## Operator wiring (one-time per network)

```bash
# 1. Open R-Chain channels (Knt↔OP_NET, …)
kntd rpc relay openChannel \
  --source <kntChainID> --dest 4294967299 \
  --ordering ordered --version 1.0

# 2. Register OP_NET feeds on O-Chain
kntd rpc oracle registerFeed \
  --name opnet:btc-tip \
...
forge script script/RegisterOPNET.s.sol --broadcast \
  -e TELEPORT_BRIDGE=0x... -e KBTC_TOKEN=0x... -e MPC_SIGNER=0x...
...
Knt ships the relayer as a chain (R-Chain / `relayvm`) and the oracle as a
...
Everything is queryable from `@kinet/graph` once the gateway is deployed:
