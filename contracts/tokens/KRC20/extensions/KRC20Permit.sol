 ...
 * @title KRC20Permit
 * @author kinet labs
 * @notice KRC-20 extension for gasless approvals via EIP-2612 (LP-3026)
 * @dev Composable extension implementing permit functionality
 */
abstract contract KRC20Permit is ERC20, IERC20Permit, EIP712, Nonces {
     ...
     */
    error KRC20PermitExpiredSignature(uint256 deadline);

    /**
     * @notice Error thrown when recovered signer doesn't match owner
     */
    error KRC20PermitInvalidSigner(address signer, address owner);
        ...
        override
    {
        if (block.timestamp > deadline) {
            revert KRC20PermitExpiredSignature(deadline);
        }
        ...
        if (signer != owner) {
            revert KRC20PermitInvalidSigner(signer, owner);
        }
