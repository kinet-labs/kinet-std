...
/**
 * @title KRC20FlashMint
 * @author kinet labs
 * @notice KRC-20 extension for flash minting (LP-3027)
 * @dev Implements EIP-3156 flash loan interface for token minting
 */
abstract contract KRC20FlashMint is ERC20, IERC3156FlashLender {
    bytes32 private constant CALLBACK_SUCCESS = keccak256("ERC3156FlashBorrower.onFlashLoan");

    /**
     * @notice Error thrown when token is not this contract
     */
    error KRC20FlashMintUnsupportedToken(address token);

    /**
     * @notice Error thrown when amount exceeds max flash loan
     */
    error KRC20FlashMintExceededMaxLoan(uint256 maxLoan);

    /**
     * @notice Error thrown when callback doesn't return success
     */
    error KRC20FlashMintCallbackFailed();

    /**
     * @notice Error thrown when repayment fails
     */
    error KRC20FlashMintRepayFailed();
    ...
    function flashFee(address token, uint256 amount) public view virtual override returns (uint256) {
        if (token != address(this)) {
            revert KRC20FlashMintUnsupportedToken(token);
        }
        ...
        returns (bool)
    {
        ...
        if (amount > maxLoan) {
            revert KRC20FlashMintExceededMaxLoan(maxLoan);
        }
        ...
        if (receiver.onFlashLoan(_msgSender(), token, amount, fee, data) != CALLBACK_SUCCESS) {
            revert KRC20FlashMintCallbackFailed();
        }
