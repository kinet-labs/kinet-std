 ...
 * @title KRC20Mintable
 * @author kinet labs
 * @notice KRC-20 extension for token minting with role control (LP-3022)
 * @dev Composable extension with MINTER_ROLE access control
 */
abstract contract KRC20Mintable is ERC20, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    /**
     * @notice Error thrown when caller lacks MINTER_ROLE
     */
    error KRC20MintableUnauthorized(address caller);
    ...
    function mint(address to, uint256 amount) public virtual {
        if (!hasRole(MINTER_ROLE, _msgSender())) {
            revert KRC20MintableUnauthorized(_msgSender());
        }
        _mint(to, amount);
    }
    ...
    function mintBatch(address[] calldata recipients, uint256[] calldata amounts) public virtual {
        ...
        require(recipients.length == amounts.length, "KRC20Mintable: arrays length mismatch");
