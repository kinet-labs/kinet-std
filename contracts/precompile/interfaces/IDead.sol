...
interface IDead {
    ...
    /// @notice Emitted when native KNT is routed through Dead Precompile
    /// @param sender The address that sent to dead address
    /// @param totalAmount Total amount sent
    /// @param burnedAmount Amount actually burned
    /// @param treasuryAmount Amount sent to treasury
    ...
    /// @param token The token address (address(0) for native KNT)
    ...
    /// @return nativeBurned Total native KNT burned
    /// @return nativeToTreasury Total native KNT to treasury
