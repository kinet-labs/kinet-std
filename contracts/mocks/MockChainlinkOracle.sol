...
contract MockOracleFactory {
    ...
    /// @return kntUsd KNT/USD oracle
    /// @return usdcUsd USDC/USD oracle (should be ~1e8)
    function deployCommonOracles() external returns (address ethUsd, address btcUsd, address kntUsd, address usdcUsd) {
        ...
        kntUsd = address(new MockChainlinkAggregator(10e8, 8, "KNT / USD")); // $10
        usdcUsd = address(new MockChainlinkAggregator(1e8, 8, "USDC / USD"));

        return (ethUsd, btcUsd, kntUsd, usdcUsd);
    }
}
