 ...
 * @title sKToken.sol
 * @author kinet labs
 * @notice ERC-4626 yield vault over a Knt K token (KUSD/KBTC/KETH/KSOL/KTON/KXRP/KDOT).
 *
 * Deposit semantics are vanilla ERC-4626 — depositors get shares (sK*) that
 * represent their pro-rata claim on the underlying K tokens held by the vault.
 *
 ...
 *      the escrowed shares (ERC-4626 redeem path) and ships the K tokens out.
 ...
 * vault as K tokens, raising the per-share value naturally.
 *
 * GOVERNANCE_ROLE can pause new stakes (existing unstakes always claim — exit
 * guarantee) and can swap the strategy.
 */
contract sKToken is ERC4626, AccessControl, Pausable, ReentrancyGuard {
    ...
    /// @param liquidToken  underlying K token
    /// @param name_        vault token name ("Staked LiquidUSD")
    /// @param symbol_      vault token symbol ("sKUSD")
    ...
    // ─────────────────────────────────────────────────────────────────────
    //  DECIMALS — mirror the underlying K token's decimals
    // ─────────────────────────────────────────────────────────────────────
    ...
    /// @notice Claim cooled-down shares as underlying K tokens.
