 ...
 * for handing harvested K tokens back to the vault via transfer; the vault
...
interface IYieldStrategy {
    /// @notice The K token this strategy farms with
    ...
    /// @notice Sum of K tokens externally deployed by this strategy (across
    ///         all integrated venues). Used by the vault to compute totalAssets.
    function externalBalance() external view returns (uint256);

    /// @notice Withdraw `amount` of K tokens from external venues back to the
