...
contract OmnichainLPRouter is ReentrancyGuard {
    ...
    address public immutable WKNT; // Wrapped KNT token
    ...
    /**
     * @dev Add liquidity with native KNT
     */
    ...
    {
        address pair = _getPairOrCreate(token, WKNT);

        (amountToken, amountKNT) =
            _calculateOptimalAmounts(token, WKNT, amountTokenDesired, msg.value, amountTokenMin, amountKNTMin);

        // Transfer token to pair
        IERC20(token).safeTransferFrom(msg.sender, pair, amountToken);

        // Wrap KNT and transfer to pair
        ...
        if (msg.value > amountKNT) {
            payable(msg.sender).transfer(msg.value - amountKNT);
        }

        emit LiquidityAdded(pair, to, amountToken, amountKNT, liquidity);
    }
